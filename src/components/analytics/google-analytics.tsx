...
import { GA_MEASUREMENT_ID, isGoogleAnalyticsEnabled, pageview } from "@/lib/analytics";
 ...
 *
 * - `isGoogleAnalyticsEnabled`(lib/analytics.ts)가 false면(측정 ID 미설정 또는
 *   프로덕션이 아님) 아무 스크립트도 렌더링하지 않는다 — 개발 중에는 GA 요청 자체가
 *   전혀 나가지 않는다. Microsoft Clarity(`microsoft-clarity.tsx`)는 완전히 독립된
 *   컴포넌트/전역 객체를 쓰므로 이 컴포넌트와 함께 렌더링해도 서로 영향을 주지 않는다.
