 ...
 *
 * Before→After를 카드 안에서 압축해 보여준다 — 실물 증거(포트폴리오)가 "품질이 이
 * 가격에 가능하다"는 확신을 주는 지점이라, 바로 다음 Pricing 섹션과 심리적으로 이어진다.
 *
 * UI Polish(2026-07-23): 포트폴리오는 이 사이트에서 가장 중요한 실물 증거라는 판단에
 * 따라 2열 그리드(카드당 실렌더폭 대비 너무 작은 33vw)를 버리고 1열 전체 폭으로
 * 확대했다. AssuranceBlock/TemplateBlock에서 이미 쓰던 "이미지 | 텍스트" 좌우 분할
 * (Desktop 60/40)·세로 스택(Mobile) 패턴을 그대로 재사용해 사이트 전체의 "증거 제시
 * 레이아웃" 언어를 통일했다 — 새로운 레이아웃을 발명하지 않고 이미 검증된 패턴을
 * 재사용한다. `sizes`도 실제 렌더 폭(데스크톱 약 55vw)에 맞게 재계산해, 2열 그리드
 * 시절 저해상도 이미지가 선택되던 문제(sizes=33vw 불일치)를 근본적으로 해결했다.
 *
